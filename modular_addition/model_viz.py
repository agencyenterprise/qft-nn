...
def viz_weights_modes(weights, P, fname, title="Modes"):
    ...
    for mode in range(n_plots):
        cos = c1[mode]
        sin = s1[mode]
        # print("cos.shape in viz_weights_modes", cos.shape)
        # print("sin.shape in viz_weights_modes", sin.shape)
        plt.subplot(num_rows, num_cols, mode + 1)
        color_idx = [(i * mode) % P for i in range(P)]
        color_for_mode = [colors[idx] for idx in color_idx]
        # print("color_for_mode.shape", len(color_for_mode))
        # print("color_for_mode", color_for_mode)
        # print(f"color_for_mode[0]: {color_for_mode[0]}")
        # print(f"color_for_mode[1]: {color_for_mode[1]}")
