...
import random
from model_mlp_ihvp import MLP_IHVP
    ...
    for i in tqdm(range(params.n_batches)):
        ...
        with t.no_grad():
            ...
            if checkpoint_every is not None and i % checkpoint_every == 0:
                t.save(
                    model.state_dict(),
                    f"models_mlp/checkpoints/{params.get_suffix(checkpoint_no)}.pt",
                )
                checkpoint_no += 1
            if i % print_every == 0:
            ...
            if i % frame_every == 0 and params.movie:
                viz_weights_modes(
                    ...
                    params.p,
                    f"frames/embeddings_movie_mlp{step:06}.png",
                )
                step += 1
        model.train()
...
def run_exp(params: ExperimentParams):
    if not os.path.exists(os.path.join("models_mlp", "checkpoints")):
        os.makedirs(os.path.join("models_mlp", "checkpoints"))
    if not os.path.exists("frames_mlp"):
        os.makedirs("frames_mlp")
    params.save_to_file(f"models_mlp/params_{params.get_suffix()}.json")
    print(f"models_mlp/params_{params.get_suffix()}.json")
    ...
    fname = f"models_mlp/model_mlp_{params.get_suffix()}.pt"
    t.save(model.state_dict(), fname)
    if params.do_viz_weights_modes:
        viz_weights_modes(
            model.embedding.weight.detach().cpu(),
            params.p,
            f"plots/final_embeddings_mlp_{params.get_suffix()}.png",
        )
    if len(mode_loss_history) > 0:
        plot_mode_ablations(
            mode_loss_history, f"plots/ablation_mlp_{params.get_suffix()}.png"
        )
    if len(magnitude_history) > 0:
        plot_magnitudes(
            magnitude_history,
            params.p,
            f"plots/magnitudes_mlp_{params.get_suffix()}.png",
        )
    if params.movie:
